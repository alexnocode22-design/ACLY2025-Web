...
Future<bool> publishNutritionPlan(
  int tempId,
  String name,
  String description,
  String price,
  ...
  try {
    ...
    final List<dynamic> tempPhotos =
        List<dynamic>.from(temp['nutrition_plan_photo'] ?? []);
    ...
    if (tempPhotos.isEmpty) {
      if (kDebugMode) print('❌ publishNutritionPlan: no photos');
      return false;
    }
    ...
    if (finalUrls.isEmpty) {
      if (kDebugMode) print('❌ publishNutritionPlan: all photo moves failed');
      return false;
    }

    final Map<String, dynamic> photoData = {
      'nutrition_plan_photo': finalUrls,
      'nutrition_plan_photo1': finalUrls.isNotEmpty ? finalUrls[0] : null,
      'nutrition_plan_photo2': finalUrls.length > 1 ? finalUrls[1] : null,
      'nutrition_plan_photo3': finalUrls.length > 2 ? finalUrls[2] : null,
      'nutrition_plan_photo4': finalUrls.length > 3 ? finalUrls[3] : null,
      'nutrition_plan_photo5': finalUrls.length > 4 ? finalUrls[4] : null,
    };

    // ── PDF ────────────────────────────────────────────────────────────────
    final String? pdfUrl = temp['nutrition_plan_file'] as String?;
        ...
        try {
          finalPdfUrl = await withRetry(() async {
            final bytes = await SupaFlow.client.storage
                .from(tempBucket)
                .download(objectPath);
            final newPath = '$trainerId/$targetId/nutrition.pdf';
            await SupaFlow.client.storage.from(permBucket).uploadBinary(
                newPath, bytes,
          ...
          if (draftType == 'edit' && sourceId != null) {
            try {
              final sourceRow = await SupaFlow.client
                  ...
                  .eq('id', sourceId)
                  .single();
              finalPdfUrl = sourceRow['nutrition_plan_file'] as String?;
            } catch (_) {
              finalPdfUrl = null;
            }
          } else {
    ...
    await withRetry(() async {
      if (draftType == 'edit' && sourceId != null) {
        await SupaFlow.client.from('nutritionPlanMarket').update({
          'nutrition_plan_name': name,
          'nutrition_plan_description': description,
          'nutrition_plan_price': int.tryParse(price),
          'nutrition_plan_file': finalPdfUrl,
          'nutrition_plan_status': true,
          'nutrition_plan_publish': true,
          ...photoData,
        }).eq('id', sourceId);
        if (kDebugMode) print('✅ Updated nutrition plan: $sourceId');
      } else {
        await SupaFlow.client.from('nutritionPlanMarket').insert({
        ...
        if (kDebugMode) print('✅ Inserted new nutrition plan');
      }
    }, label: 'DB write');
    ...
    if (kDebugMode) print('✅ publishNutritionPlan complete');
    return true;
  } catch (e) {
    if (kDebugMode) print('❌ publishNutritionPlan error: $e');
    return false;
  }
}
