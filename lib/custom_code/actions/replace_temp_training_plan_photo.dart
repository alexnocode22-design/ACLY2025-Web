...
Future<String> replaceTempTrainingPlanPhoto(
  int tempId,
  int photoIndex,
  ...
  try {
    ...
    if (bytes == null || bytes.isEmpty) {
      if (kDebugMode) print('❌ No file bytes in replaceTempTrainingPlanPhoto');
      return '';
    }
    ...
    final String trainerId = row['trainer_id'];

    if (kDebugMode)
      print(
          '🔍 replaceTempTrainingPlanPhoto: index=$photoIndex count=${currentPhotos.length} bytes=${bytes.length}');
    ...
    if (kDebugMode)
      print('✅ Plan photo replaced at index $photoIndex: $newUrl');
    return newUrl;
  } catch (e) {
    if (kDebugMode) print('❌ replaceTempTrainingPlanPhoto error: $e');
    return '';
  }
}
