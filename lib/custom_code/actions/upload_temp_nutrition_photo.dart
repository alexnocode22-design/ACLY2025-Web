...
Future<String> uploadTempNutritionPhoto(
  int tempId,
  ...
  try {
    ...
    await SupaFlow.client.storage.from('acly-nutritionplans-temp').uploadBinary(
        path, bytes,
        fileOptions:
            const FileOptions(contentType: 'image/jpeg', upsert: true));

    final String url = SupaFlow.client.storage
        .from('acly-nutritionplans-temp')
        .getPublicUrl(path);
    ...
    if (kDebugMode) print('✅ Nutrition photo uploaded: $url');
    return url;
  } catch (e) {
    if (kDebugMode) print('❌ uploadTempNutritionPhoto error: $e');
    return '';
  }
}
